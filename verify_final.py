...
res = run_remote("cd /home/orlovrp/flipoly && sudo docker compose ps && sudo docker compose -f docker-compose.live-v2.yml --profile live-v2 ps")
print(res.stdout)
if res.stderr: print("STDERR:", res.stderr)

print("\n=== Check API Logs ===")
res = run_remote("cd /home/orlovrp/flipoly && sudo docker compose logs --tail=50 api | grep -iE 'Traceback|ERROR|500|exception|failed'")
print(res.stdout)

print("\n=== Check Live Worker Logs ===")
res = run_remote("cd /home/orlovrp/flipoly && sudo docker compose -f docker-compose.live-v2.yml --profile live-v2 logs --tail=50 execution_worker_live | grep -iE 'Traceback|ERROR|500|exception|failed'")
print(res.stdout)
