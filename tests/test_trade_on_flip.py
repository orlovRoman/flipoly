...
async def test_no_trade_when_flip_threshold_met(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="OUTSIDER_MAX_PRICE", value="0.60", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_BET_EDGE", value="2.0", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_EDGE_FILTER", value="2.0", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_PRICE_DRIFT", value="0.20", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
    ...
    with patch("polyflip.trading.engine.PolymarketClient") as mock_api_cls:
        ...
        mock_api.get_market_prices = AsyncMock(
            side_effect=lambda token_id: (
                ...
                if token_id == "t_yes"
                else {"best_ask": 0.32, "best_bid": 0.30}
            )
        )
        ...
        ), f"Trade was skipped: {trades[0].error_msg}"
...
async def test_no_trade_skipped_when_price_exceeds_max(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="NO_MIN_EDGE", value="0.04", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_BET_EDGE", value="2.0", updated_at=now, updated_by="test"
        ),
    ...
    with patch("polyflip.trading.engine.PolymarketClient") as mock_api_cls:
        ...
        mock_api.get_market_prices = AsyncMock(
            side_effect=lambda token_id: (
                ...
                if token_id == "t_yes"
                else {"best_ask": 0.65, "best_bid": 0.63}
            )
        )
        ...
        assert (
            "max_outsider_price" in trades[0].error_msg
            or "Price drift" in trades[0].error_msg
            or "OUTSIDER strategy selected a favorite token" in trades[0].error_msg
        )
...
async def test_no_trade_skipped_when_flip_prob_too_small(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="OUTSIDER_MAX_PRICE", value="0.75", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="NO_MIN_EDGE", value="0.05", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_BET_EDGE", value="2.0", updated_at=now, updated_by="test"
        ),
    ...
    with patch("polyflip.trading.engine.PolymarketClient") as mock_api_cls:
        ...
        mock_api.get_market_prices = AsyncMock(
            side_effect=lambda token_id: (
                ...
                if token_id == "t_yes"
                else {"best_ask": 0.31, "best_bid": 0.29}
            )
        )
        ...
        assert (
            "dead" in err or "zone" in err or "мёртв" in err or "зона" in err
