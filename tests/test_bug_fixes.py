...
def test_crypto_action_trade_should_execute():
    ...
    decision = TradeDecision(action='BUY_YES', buy_price=0.4, bet_size_usdc=5.0, strategy_type='LIGHTGBM_TREND', reason='test', edge=0.12)
    assert (not decision or decision.action != 'SKIP') is True, 'BUG-01: условие должно быть True для BUY_YES (блок инициализации p_flip/edge)'
...
def test_crypto_action_skip_should_skip_init_block():
    ...
    from polyflip.trading.decision_logic import TradeDecision
    decision = TradeDecision(action='SKIP', buy_price=0.0, bet_size_usdc=0.0, strategy_type='LIGHTGBM_TREND', reason='No signal', edge=0.0)
    assert (not decision or decision.action != 'SKIP') is False, 'BUG-01: условие должно быть False для SKIP (блок инициализации не запускается)'
...
def test_engine_py_no_action_trade_condition():
    ...
    assert 'decision_obj.action != "trade"' not in source, "BUG-01 PRESENT: найдено инвертированное условие action != 'trade' в engine.py! Должно быть: action != 'SKIP'"
...
def test_run_backtest_epsilon_not_zero():
    ...
    assert 'epsilon=0.0,' not in source, 'BUG-02: epsilon=0.0 захардкожен! Используй константу _EPSILON = 1e-9'
...
def test_stoploss_job_has_try_except():
    ...
    assert 'try:' in job_body, 'BUG-03: stoploss_job не имеет try/except — необработанное исключение упадёт в планировщик'
    assert 'except Exception' in job_body, 'BUG-03: stoploss_job не перехватывает Exception'
...
async def test_stoploss_job_handles_exception():
    ...
    api_mock = AsyncMock()
    with patch('polyflip.scheduler.jobs.stoploss_worker_cycle', side_effect=RuntimeError('DB connection failed')):
        try:
...
def test_tp_worker_uses_bid_not_ask():
    ...
    assert 'best_ask' not in source, 'BUG-04: TP-воркер использует best_ask! Для продажи токенов на Polymarket нужен best_bid (цена покупателей)'
...
def test_tp_function_uses_current_bid_param():
    ...
    assert 'current_bid' in params, f'BUG-04: evaluate_take_profit не имеет параметра current_bid. Параметры: {params}'
    assert 'current_ask' not in params, f'BUG-04: evaluate_take_profit всё ещё имеет current_ask. Параметры: {params}'