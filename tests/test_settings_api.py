...
async def test_update_setting_valid_daily_limit(db_session):
    ...
    try:
        await update_setting(
            "DAILY_LOSS_LIMIT_USDC", SettingValue(value="-250"), db=db_session
        )
...
async def test_update_setting_invalid_daily_limit(db_session):
    ...
    try:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "DAILY_LOSS_LIMIT_USDC", SettingValue(value="0"), db=db_session
            )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "DAILY_LOSS_LIMIT_USDC", SettingValue(value="50"), db=db_session
            )
        assert exc_info.value.status_code == 400
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "DAILY_LOSS_LIMIT_USDC", SettingValue(value="-150000"), db=db_session
            )
        assert exc_info.value.status_code == 400
    finally:
        settings_module.async_session = original_session
...
async def test_update_setting_valid_poll_interval(db_session):
    ...
    try:
        await update_setting(
            "LIVE_POLL_INTERVAL_SECONDS", SettingValue(value="15"), db=db_session
        )
...
async def test_update_setting_invalid_poll_interval(db_session):
    ...
    try:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "LIVE_POLL_INTERVAL_SECONDS", SettingValue(value="1"), db=db_session
            )
        assert exc_info.value.status_code == 400
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "LIVE_POLL_INTERVAL_SECONDS", SettingValue(value="301"), db=db_session
            )
        assert exc_info.value.status_code == 400
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "LIVE_POLL_INTERVAL_SECONDS", SettingValue(value="abc"), db=db_session
            )
        assert exc_info.value.status_code == 400
    finally:
        settings_module.async_session = original_session
...
async def test_update_settings_bulk(db_session):
    ...
    try:
        payload = BulkSettings(
            settings={"TRADE_BET_SIZE_USDC": "25.0", "TRADE_MAX_PRICE": "0.95"}
        )
