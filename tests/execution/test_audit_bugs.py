...
async def test_daily_loss_limit(db_session: AsyncSession):
    ...
    db_session.add(
        RuntimeSettings(
            key="DAILY_LOSS_LIMIT_USDC", value="-100", updated_at=now, updated_by="test"
        )
    )
...
async def test_released_reservation_not_counted_in_pretrade(db_session: AsyncSession):
    now = datetime.datetime.now(datetime.timezone.utc)
    market = LiveMarket(
        market_id="m1", asset="a", yes_token_id="y1", no_token_id="n1", question="q"
    )
    # Set exposure limit to 50
    db_session.add(
        RuntimeSettings(
            key="MAX_TOTAL_EXPOSURE_USDC", value="50", updated_at=now, updated_by="test"
        )
    )
    ...
    val = await validate_pre_trade(
        db_session, MockClient(), market, decision, cfg, "LIGHTGBM", 0.05, 0.99, 0.6, 1
    )
    assert (
        val.valid is True
    ), f"Blocked by reservation that is released: {val.skip_reason}"
