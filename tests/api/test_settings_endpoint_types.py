...
from polyflip.api.settings import (
...
def test_settings_endpoints_accept_none_request():
    for fn in [update_settings_bulk, update_setting, update_security_setting]:
        ...
        ), f"{fn.__name__}: request должен быть Optional[Request]"
        assert (
            type(None) in args
        ), f"{fn.__name__}: None должен быть допустимым типом для request"
        assert (
            Request in args
        ), f"{fn.__name__}: Request должен быть допустимым типом для request"
