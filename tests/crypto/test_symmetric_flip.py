...
async def test_symmetric_flip_rules():
    ...
    # Valid
    dec = decide_crypto_trend(
        sig_down_ok, 0.55, 1000.0, config, p_flip_ml=0.65, no_ask=0.5
    )
    assert dec.action == "BUY_NO"
    # Blocked by low flip prob (< 0.60)
    dec = decide_crypto_trend(
        sig_down_ok, 0.55, 1000.0, config, p_flip_ml=0.50, no_ask=0.5
    )
    assert dec.action == "SKIP"
    ...
    # Valid
    dec = decide_crypto_trend(
        sig_down_ok, 0.45, 1000.0, config, p_flip_ml=0.20, no_ask=0.6
    )
    assert dec.action == "BUY_NO"
    # Blocked
    dec = decide_crypto_trend(
        sig_down_ok, 0.45, 1000.0, config, p_flip_ml=0.40, no_ask=0.6
    )
    assert dec.action == "SKIP"
    assert "Fav trade blocked" in dec.reason
    ...
    dec_none_down = decide_crypto_trend(
        sig_down_ok, 0.55, 1000.0, config, p_flip_ml=None
    )
