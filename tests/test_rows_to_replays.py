...
def _make_row(market_id="mkt-1", asset="BTC", minutes_left=30.0, outcome="YES", idx=0):
...
class TestRowsToReplays:
    ...
    def test_filters_non_tradeable(self):
        ...
        rows_yes = [
            _make_row("mkt-yes", outcome="YES", minutes_left=30 - i, idx=i)
            for i in range(3)
        ]
        rows_pending = [
            _make_row("mkt-pending", outcome="PENDING", minutes_left=30 - i, idx=i + 3)
            for i in range(3)
        ]
        rows_invalid = [
            _make_row("mkt-invalid", outcome="INVALID", minutes_left=30 - i, idx=i + 6)
            for i in range(3)
        ]
    ...
    def test_min_snapshots_filter(self):
        ...
        rows_small = [
            _make_row("mkt-small", minutes_left=30 - i, idx=i + 5) for i in range(2)
        ]

        replays = rows_to_replays(rows_big + rows_small, min_snapshots=3)
        assert "mkt-big" in replays
        assert (
            "mkt-small" not in replays
        ), "Markets with < min_snapshots should be excluded"

    def test_ticks_sorted_descending_time_left(self):
        """MarketReplay.ticks отсортированы от max к min time_left_min."""
        rows = [
            _make_row(minutes_left=float(t), idx=i)
            for i, t in enumerate([5, 20, 10, 30, 15])
        ]
    ...
    def test_multiple_markets(self):
        rows = (
            ...
            + [
                _make_row("mkt-C", minutes_left=30 - i, idx=i + 8, outcome="NO")
                for i in range(4)
            ]
        )
        replays = rows_to_replays(rows)
