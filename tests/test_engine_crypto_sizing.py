...
def test_validator_features_subset_of_columns():
    ...
    assert set(CryptoFeaturesValidator.model_fields.keys()).issubset(
        set(CRYPTO_FEATURE_COLUMNS)
    )
...
def test_crypto_predictor_cache():
    ...
    predictor._model_versions["BTCUSDT"] = {
        "low_vol": 42,
        "mid_vol": 42,
        "high_vol": 42,
    }
    predictor._thresholds["BTCUSDT"] = {
        "low_vol": (0.55, 0.45),
        "mid_vol": (0.55, 0.45),
        "high_vol": (0.55, 0.45),
    }
    ...
    async def run_test():
        ...
        mock_result.all.return_value = [
            MockRow("BTCUSDT_low_vol", 42),
            MockRow("BTCUSDT_mid_vol", 42),
            MockRow("BTCUSDT_high_vol", 42),
        ]
...
async def test_engine_crypto_standalone_bet_size(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="TRADE_MIN_TIME_LEFT_SEC", value="10", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="TRADE_MAX_TIME_LEFT_SEC",
            value="1000",
            updated_at=now,
            updated_by="test",
        ),
        RuntimeSettings(
            key="TRADING_MODE_BTC", value="lightgbm", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="CRYPTO_MIN_EDGE", value="0.05", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="CRYPTO_THRESHOLD_UP_BTC",
            value="0.60",
            updated_at=now,
            updated_by="test",
        ),
        RuntimeSettings(
            key="CRYPTO_THRESHOLD_DOWN_BTC",
            value="0.40",
            updated_at=now,
            updated_by="test",
        ),
        RuntimeSettings(
            key="MAX_BET_SIZE_USDC", value="50.0", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="TRADE_BET_SIZE_USDC", value="10.0", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="BYPASS_BET_SIZE_CHECK", value="true", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="MAX_PRICE_DRIFT", value="0.05", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
    ...
    with patch("polyflip.trading.engine.PolymarketClient") as mock_api_cls, patch(
        "pickle.loads", return_value=mock_model
    ), patch(
        "polyflip.crypto.predictor.build_crypto_features", return_value=mock_features
    ), patch(
        "polyflip.trading.decision_runners.get_recent_candles",
        AsyncMock(return_value=fake_candles),
    ):
