...
class MockSnapshot:
    ...
    def mid_price(self):
        return (
            (self.best_bid_yes + self.best_ask_yes) / 2
...
def test_full_backtest_integration():
    ...
    snapshots = [
        ...
        MockSnapshot(
            "m1", "BTC", 50, 0.60, 0.40, 0.59, 0.61, "YES", now - timedelta(minutes=50)
        ),
        MockSnapshot(
            "m1", "BTC", 30, 0.70, 0.30, 0.69, 0.71, "YES", now - timedelta(minutes=30)
        ),
        MockSnapshot(
            "m1", "BTC", 10, 0.85, 0.15, 0.84, 0.86, "YES", now - timedelta(minutes=10)
        ),
        # Market 2: Outsider flip (LOSS)
        MockSnapshot(
            "m2", "ETH", 45, 0.80, 0.20, 0.79, 0.81, "NO", now - timedelta(minutes=45)
        ),
        MockSnapshot(
            "m2", "ETH", 25, 0.60, 0.40, 0.59, 0.61, "NO", now - timedelta(minutes=25)
        ),
        MockSnapshot(
            "m2", "ETH", 5, 0.40, 0.60, 0.39, 0.41, "NO", now - timedelta(minutes=5)
        ),
    ]
