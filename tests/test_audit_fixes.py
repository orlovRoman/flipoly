...
async def test_bug_01_zero_threshold_ignored(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="TRADE_FLIP_THRESHOLD_BTC", value="0", updated_at=now, updated_by="test"
        ),  # BUG-01: Наш нуль
    ]
    ...
    trader_mock.execute_trade = AsyncMock(
        return_value={"status": "SUCCESS", "error_msg": None}
    )

    api_client_mock = AsyncMock()
    api_client_mock.get_market_prices = AsyncMock(
        return_value={
...
async def test_bug_02_auto_threshold_keys_loaded(db_session):
    ...
    settings = [
        ...
        RuntimeSettings(
            key="AUTO_FLIP_THRESHOLD_BTC_decided",
            value="0.20",
            updated_at=now,
            updated_by="test",
        ),
...
async def test_bug_04_retrain_job_uses_db_assets(db_session):
    ...
    db_session.add(
        RuntimeSettings(
            key="TRADE_ASSETS", value="DOGE,XRP", updated_at=now, updated_by="test"
        )
    )
    await db_session.commit()
    ...
    with patch("polyflip.scheduler.jobs.ModelTrainer") as mock_trainer_cls, patch(
        "polyflip.scheduler.jobs.settings"
    ) as mock_settings, patch(
        "polyflip.scheduler.jobs.async_session", return_value=mock_session_cm
    ):
        ...
        called_assets = [
            call.args[0] for call in mock_trainer.train_model.call_args_list
        ]
...
async def test_bug_05_06_group_shuffle_split(db_session):
    ...
    for i in range(100):
        ...
        snaps.append(
            MarketSnapshot(
                market_id=market_id,
                asset="BTC",
                time_left_min=float(i % 10 + 1),
                mid_price=0.8 if i % 2 == 0 else 0.2,
                spread=0.01,
                volume_5min=100.0,
                price_velocity=0.0,
                hour_of_day=12,
    ...
    with patch.object(trainer_settings, "MIN_SAMPLES_FOR_MODEL", 10), patch.object(
        trainer_settings, "ACTIVE_FEATURES", "mid_price,spread,time_left_min"
    ), patch(
        "polyflip.models.trainer.GroupShuffleSplit", wraps=GroupShuffleSplit
    ) as mock_gss:
...
async def test_bug_09_pnl_polymarket_fee(db_session):
    ...
    q_win = await db_session.execute(
        select(TradeHistory).where(TradeHistory.market_id == "m_win")
    )
    t_win = q_win.scalar_one()

    q_lose = await db_session.execute(
        select(TradeHistory).where(TradeHistory.market_id == "m_lose")
    )
