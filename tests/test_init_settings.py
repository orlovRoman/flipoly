...
def test_defaults_max_edge_matches_constant():
    """DEFAULTS["MAX_BET_EDGE"] должен совпадать с constants.MAX_EDGE_SCALING."""
    from polyflip.db.init_runtime_settings import DEFAULTS
    from polyflip.constants import MAX_EDGE_SCALING as MAX_BET_EDGE
    assert float(DEFAULTS["MAX_BET_EDGE"]) == MAX_BET_EDGE, (
        f"DEFAULTS['MAX_BET_EDGE']={DEFAULTS['MAX_BET_EDGE']} "
        f"не совпадает с constants.MAX_EDGE_SCALING={MAX_BET_EDGE}"
    )
...
def test_max_edge_is_higher_than_realistic_signals():
    ...
    from polyflip.constants import MAX_EDGE_SCALING as MAX_BET_EDGE, MIN_EDGE
