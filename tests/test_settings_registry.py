...
def test_no_orphan_settings_db_keys_in_engine():
    ...
    assert not orphans, f"Ключи, которые engine читает, но нет в реестре: {orphans}"
...
@pytest.mark.asyncio
async def test_get_all_settings_includes_security_sensitive_keys_as_masked(
...
def test_no_settingdef_uses_settingmeta():
    ...
    assert not hasattr(
        sr, "SettingMeta"
    ), "SettingMeta не существует, используй SettingDef"
