...
def test_backtest_respects_lgbm_params(sample_df):
    ...
    r1 = run_backtest(
        sample_df, "TEST", lgbm_params={"num_leaves": 4, "n_estimators": 10}
    )
    r2 = run_backtest(
        sample_df, "TEST", lgbm_params={"num_leaves": 127, "n_estimators": 100}
    )
...
def test_edge_rate_below_50_pct_after_fix(sample_df):
    ...
    assert (
        result.edge_rate < 0.50
    ), f"edge_rate={result.edge_rate:.1%} — фильтры не работают"
