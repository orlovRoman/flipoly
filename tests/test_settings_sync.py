...
def test_settings_sync():
        ...
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Attribute) and node.func.attr == "get":
                if (
                    ...
                    and node.func.value.id == "settings_db"
                ):
                    if (
                        node.args
                        ...
                        if (
                            not key.startswith("TRADING_MODE_")
                            and not key.startswith("MIN_EDGE_")
                            and not key.startswith("TRADE_MAX_PRICE_")
                        ):
                            engine_keys.add(key)
    ...
    ), f"Ключи, используемые в engine.py, но отсутствующие в settings_registry.py: {missing_in_registry}"
