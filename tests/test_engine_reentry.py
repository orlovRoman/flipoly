...
async def test_skipped_then_signal_appears(db_session):
    ...
    db_settings = [
        ...
        RuntimeSettings(
            key="TRADE_ASSETS", value="BTC", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
        ...
        RuntimeSettings(
            key="MAX_BET_EDGE", value="1.50", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
    ...
    res = await db_session.execute(
        select(TradeHistory).where(TradeHistory.market_id == "m_reentry")
    )
    ...
    res_reg = await db_session.execute(
        select(ModelRegistry).where(ModelRegistry.asset == "BTC")
    )
...
async def test_no_double_entry(db_session):
    ...
    db_settings = [
        ...
        RuntimeSettings(
            key="TRADE_ASSETS", value="ETH", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="ACTIVE_FEATURES", value="mid_price", updated_at=now, updated_by="test"
        ),
    ...
    res = await db_session.execute(
        select(TradeHistory).where(TradeHistory.market_id == "m_double")
    )
    trades = res.scalars().all()
    assert len(trades) == 1
    assert trades[0].status == "SUCCESS"
    ...
    res_reg = await db_session.execute(
        select(ModelRegistry).where(ModelRegistry.asset == "ETH")
    )
    registry = res_reg.scalar_one()
