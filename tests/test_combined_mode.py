...
def test_evaluate_combined_entry_direction_up_success():
    ...
    expected_gross = 0.8 - 0.62
    expected_net = round(expected_gross - 0.03, 4)
    assert math.isclose(res.gross_edge, expected_gross, rel_tol=0.0001)
    assert math.isclose(res.net_edge, expected_net, rel_tol=0.0001)
...
def test_evaluate_combined_entry_direction_invalid_features():
    ...
    sig = CryptoSignal(symbol='BTCUSDT', p_up=0.7, p_down=0.3, direction='UP', signal_strength=0.4, strike=65000.0, threshold_up=0.55, threshold_down=0.45, model_version=2, features_ok=False, risk_vetoed=False, status='INSUFFICIENT_CANDLES')
    res = evaluate_combined_entry(crypto_sig=sig, market_phase='mid_vol', entry_requested_key='BTC_mid_vol', entry_model_key='BTC_mid_vol', entry_model_version=5, entry_model_source='PHASE', p_flip=0.2, fresh_yes_price=0.6, yes_ask=0.62, no_ask=0.38, time_left_sec=300.0, cfg=_make_cfg())
...
def test_evaluate_combined_entry_direction_risk_vetoed():
    ...
    sig = CryptoSignal(symbol='BTCUSDT', p_up=0.7, p_down=0.3, direction='UP', signal_strength=0.4, strike=65000.0, threshold_up=0.55, threshold_down=0.45, model_version=2, features_ok=True, risk_vetoed=True, risk_reason='Extreme funding rate')
    res = evaluate_combined_entry(crypto_sig=sig, market_phase='mid_vol', entry_requested_key='BTC_mid_vol', entry_model_key='BTC_mid_vol', entry_model_version=5, entry_model_source='PHASE', p_flip=0.2, fresh_yes_price=0.6, yes_ask=0.62, no_ask=0.38, time_left_sec=300.0, cfg=_make_cfg())
    assert res.action == 'SKIP'
...
def test_evaluate_combined_entry_insufficient_net_edge():
    ...
    sig = CryptoSignal(symbol='BTCUSDT', p_up=0.75, p_down=0.25, direction='UP', signal_strength=0.5, strike=65000.0, threshold_up=0.55, threshold_down=0.45, model_version=2, features_ok=True, risk_vetoed=False)
    res = evaluate_combined_entry(crypto_sig=sig, market_phase='mid_vol', entry_requested_key='BTC_mid_vol', entry_model_key='BTC_mid_vol', entry_model_version=5, entry_model_source='PHASE', p_flip=0.35, fresh_yes_price=0.64, yes_ask=0.64, no_ask=0.36, cost_buffer=0.03, time_left_sec=300.0, cfg=_make_cfg(OUTS_MIN_EDGE='0.03'))
    assert res.action == 'SKIP'
...
def test_decide_combined_mode_full_flow():
    ...
    api_client.get_market_prices.return_value = {'current_yes_price': '0.60', 'best_ask': '0.62', 'current_spread': '0.02'}
    market = MagicMock()
    ...
    crypto_sig = CryptoSignal(symbol='BTCUSDT', model_key='BTCUSDT_mid_vol', p_up=0.85, p_down=0.15, direction='UP', signal_strength=0.7, strike=65000.0, threshold_up=0.55, threshold_down=0.45, model_version=10, features_ok=True, risk_vetoed=False, regime='MID_VOL', status='OK')
    crypto_predictor = MagicMock()
    with patch('polyflip.trading.decision_runners._fetch_lgbm_signal', AsyncMock(return_value=crypto_sig)), patch('polyflip.trading.decision_runners.infer_flip_for_market', AsyncMock(return_value=0.15)), patch('polyflip.trading.decision_runners.log_funnel', AsyncMock()) as mock_log_funnel:
        res = asyncio.run(decide_combined_mode(db_session=db_session, api_client=api_client, market=market, cfg=cfg, raw_settings=raw_settings, models_cache=models_cache, crypto_predictor=crypto_predictor, start_time=MagicMock(), time_left_sec=300.0, execution_mode='PAPER'))
