...
def test_combined_skip_preserves_edge():
    ...
    ml_res = DecisionResult(
        decision_obj=ml_dec, p_flip=0.12, model_ver=4, edge=0.08, skip_reason=None
    )
    ...
    with patch(
        "polyflip.trading.decision_runners.decide_ml_mode",
        AsyncMock(return_value=ml_res),
    ), patch(
        "polyflip.trading.decision_runners._fetch_lgbm_signal",
        AsyncMock(return_value=lgbm_proxy),
    ):
...
def test_combined_bet_reduction_original_bet():
    ...
    with patch("polyflip.trading.decision_runners.logger") as mock_logger, patch(
        "polyflip.trading.decision_runners.decide_ml_mode",
        AsyncMock(return_value=ml_res),
    ), patch(
        "polyflip.trading.decision_runners._fetch_lgbm_signal",
    ...
    bet_reduced_calls = [
        c for c in info_calls if c.args and c.args[0] == "combined_bet_reduced"
    ]
...
def test_recommended_thresholds_empty_and_invalid_values():
    ...
    db_settings = [
        ...
        RuntimeSettings(
            key="TRADE_FLIP_THRESHOLD_BTC", value="", updated_at=now, updated_by="test"
        ),
        RuntimeSettings(
            key="AUTO_FLIP_THRESHOLD_BTC",
            value="0.65",
            updated_at=now,
            updated_by="test",
        ),
    ...
    with patch(
        "polyflip.api.settings.async_session", dummy_session_creator
