...
async def test_backfill_dry_run_and_apply_keyset_pagination(db_session):
    ...
    t1 = TradeHistory(
        ...
        status="SUCCESS",
        lgbm_metadata=json.dumps(
            {
                "ml_phase_model": "BTCUSDT_low_vol",
                "lgbm_model_key": "BTCUSDT_low_vol",
                "lgbm_version": 2,
            }
        ),
    ...
    stats_dry = await run_backfill(
        apply_changes=False, mode="PAPER", session_override=db_session
    )
    ...
    # 2. Проверяем apply
    stats_apply = await run_backfill(
        apply_changes=True, mode="PAPER", session_override=db_session
    )
    await db_session.refresh(t1)
...
async def test_funnel_ambiguous_matching_not_assigned(db_session):
    ...
    f1 = DecisionFunnelLog(
        market_id="m_ambig",
        asset="DOGE",
        trading_mode="ML",
        final_action="BUY_YES",
        used_model="DOGE_leaning",
        created_at=now,
    )
    f2 = DecisionFunnelLog(
        market_id="m_ambig",
        asset="DOGE",
        trading_mode="ML",
        final_action="BUY_YES",
        used_model="DOGE_decided",
        created_at=now,
    )
...
async def test_backfill_multiple_batches_and_idempotency(db_session, monkeypatch):
    ...
    t1 = TradeHistory(
        ...
        status="SUCCESS",
        lgbm_metadata=json.dumps({"ml_phase_model": "ETHUSDT_low_vol"}),
        created_at=now,
    )
    t2 = TradeHistory(
        market_id="b2",
...
async def test_crypto_model_pnl_isolation(db_session):
    ...
    t1 = TradeHistory(
        ...
        confirm_model_key="BTCUSDT_high_vol",
        confirm_model_version=1,
        model_attribution_source="EXACT",
        mode="PAPER",
        position_status="CLOSED",
        realized_pnl_usdc=8.0,
        status="SUCCESS",
        created_at=datetime.now(timezone.utc),
    )
    db_session.add(t1)
...
async def test_delete_archive_model(db_session):
    ...
    active_summary = await get_active_models_summary(
        requested_mode="PAPER", db=db_session
    )
