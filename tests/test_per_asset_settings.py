...
async def test_update_per_asset_trading_mode(db_session):
    ...
    try:
        ...
        await update_setting(
            "TRADING_MODE_BTC", SettingValue(value="ml"), db=db_session
        )
        ...
        await update_setting(
            "TRADING_MODE_DOGE", SettingValue(value="favorite"), db=db_session
        )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "TRADING_MODE_BTC", SettingValue(value="invalid_mode"), db=db_session
            )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "TRADING_MODE_INVALIDASSET", SettingValue(value="ml"), db=db_session
            )
        assert exc_info.value.status_code == 400
    finally:
...
async def test_update_per_asset_min_edge(db_session):
    ...
    try:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "MIN_EDGE_BTC", SettingValue(value="105"), db=db_session
            )
        assert exc_info.value.status_code == 400

        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "MIN_EDGE_BTC", SettingValue(value="-1"), db=db_session
            )
        assert exc_info.value.status_code == 400
    finally:
        settings_module.async_session = original_session
...
async def test_update_per_asset_max_price(db_session):
    ...
    try:
        # Valid values
        await update_setting(
            "TRADE_MAX_PRICE_BTC", SettingValue(value="0.85"), db=db_session
        )
        stmt = select(RuntimeSettings).where(
            RuntimeSettings.key == "TRADE_MAX_PRICE_BTC"
        )
        row = (await db_session.execute(stmt)).scalar_one()
        assert float(row.value) == 0.85

        await update_setting(
            "TRADE_MAX_PRICE_BTC", SettingValue(value=""), db=db_session
        )
        stmt = select(RuntimeSettings).where(
            RuntimeSettings.key == "TRADE_MAX_PRICE_BTC"
        )
        ...
        with pytest.raises(HTTPException) as exc_info:
            await update_setting(
                "TRADE_MAX_PRICE_BTC", SettingValue(value="1.20"), db=db_session
            )
        assert exc_info.value.status_code == 400
    finally:
        settings_module.async_session = original_session
...
async def test_per_asset_empty_trading_mode_uses_global_not_empty_string(db_session):
    ...
    try:
        ...
        db_session.add(
            RuntimeSettings(
                key="TRADING_MODE_BTCUSDT", value="", updated_by="test", updated_at=now
            )
        )
        db_session.add(
            RuntimeSettings(
                key="TRADING_MODE", value="lightgbm", updated_by="test", updated_at=now
            )
        )
        ...
        mode = settings_db.get("TRADING_MODE_BTCUSDT") or settings_db.get(
            "TRADING_MODE", "ML"
        )
        ...
        assert (
            mode == "lightgbm"
        ), f"Ожидали lightgbm (global fallback), получили: {mode!r}"
