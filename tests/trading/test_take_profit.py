...
async def test_tp_worker_triggers_when_price_reached(db_session):
    ...
    req = (
        await db_session.execute(
            select(ExecutionRequest).where(ExecutionRequest.trade_history_id == t.id)
        )
    ).scalar_one_or_none()
...
async def test_tp_worker_skips_when_disabled(db_session):
    ...
    db_session.add(
        RuntimeSettings(
            key="TAKE_PROFIT_ENABLED", value="false", updated_at=now, updated_by="test"
        )
    )
    await db_session.commit()
