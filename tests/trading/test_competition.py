...
async def prep_competition_trade(db_session: AsyncSession):
    ...
    db_session.add(
        RuntimeSettings(
            key="STOP_LOSS_ENABLED", value="true", updated_at=now, updated_by="test"
        )
    )
    db_session.add(
        RuntimeSettings(
    ...
    db_session.add(
        RuntimeSettings(
            key="POLYMARKET_FEE_RATE", value="0.0", updated_at=now, updated_by="test"
        )
    )
...
@pytest.mark.skip(reason="No execute_trade call count anymore")
@pytest.mark.asyncio
async def test_step7_competition_stoploss_and_takeprofit(
    db_session: AsyncSession, prep_competition_trade
):
    """
    ...
    async def mock_execute_trade(*args, **kwargs):
        ...
        return make_dummy_execution(
            mode="PAPER", executed_price=0.55, executed_usdc=11.0
        )
    ...
    slw.evaluate_stop_loss = MagicMock(
        return_value=StopLossDecision(
            should_sell=True, stop_price=0.40, current_price=0.55, reason="test"
        )
    )
    tpw.evaluate_take_profit = MagicMock(
        return_value=TakeProfitDecision(
            should_sell=True, tp_price=0.60, current_price=0.55, reason="test"
        )
    )
