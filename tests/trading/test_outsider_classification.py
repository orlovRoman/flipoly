...
def test_outsider_decision_has_correct_strategy_type():
    ...
    assert decision.p_win_effective == pytest.approx(
        decision.decision_details["p_flip_effective"] * 0.65
    )
...
async def test_pre_trade_validator_blocks_favorite_on_outsider_token():
    ...
    decision = TradeDecision(
        ...
        bet_size_usdc=10.0,
        reason="test",
        strategy_type="PURE_FAVORITE",
        edge=0.1,
        p_win_effective=0.30,
    )
