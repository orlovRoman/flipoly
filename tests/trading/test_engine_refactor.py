...
import polyflip.db.execution_models
...
def _make_market(**kwargs):
    ...
    m.end_time_est = kwargs.get('end_time_est', datetime.now(timezone.utc) + timedelta(minutes=10))
    return m
...
class TestStep1LoadTradingSettings:
    ...
    async def test_step1_per_asset_keys_loaded(self, db_session):
        ...
        from polyflip.trading.settings_loader import load_trading_settings
        now = datetime.now(timezone.utc)
        db_session.add(RuntimeSettings(key='OUTS_MIN_EDGE_BTC', value='0.10', updated_at=now, updated_by='test'))
        db_session.add(RuntimeSettings(key='TRADING_MODE_BTC', value='crypto', updated_at=now, updated_by='test'))
        await db_session.commit()
...
class TestStep3LoadEligibleMarkets:
    ...
    async def test_step3_daily_limit_returns_none(self, db_session):
        ...
        from polyflip.trading.trading_config import parse_trading_settings
        now = datetime.now(timezone.utc)
        db_session.add(TradeHistory(market_id='m_loss', asset='BTC', outcome_bought='YES', amount_usdc=50.0, remaining_shares=20.0, executed_price=0.6, predicted_flip_prob=0.7, active_features='mid_price', status='SUCCESS', pnl=-150.0, mode='PAPER', created_at=now))
        await db_session.commit()
    ...
    async def test_step3_market_in_window_returned(self, db_session):
        ...
        db_session.add(LiveMarket(market_id='m_window', asset='BTC', question='Q?', yes_token_id='ty', no_token_id='tn', current_yes_price=0.6, current_no_price=0.4, current_spread=0.02, volume_5min=100.0, price_velocity=0.0, end_time_est=end_time, last_updated=now))
        await db_session.commit()
        cfg = parse_trading_settings(_make_raw_settings({'FAVOR_MIN_TIME_LEFT_SEC': '300', 'FAVOR_MAX_TIME_LEFT_SEC': '900'}))
        result = await load_eligible_markets(db_session, cfg, now)
    ...
    async def test_step3_market_outside_window_excluded(self, db_session):
        ...
        db_session.add(LiveMarket(market_id='m_outside', asset='BTC', question='Q?', yes_token_id='ty2', no_token_id='tn2', current_yes_price=0.6, current_no_price=0.4, current_spread=0.02, volume_5min=100.0, price_velocity=0.0, end_time_est=end_time, last_updated=now))
        await db_session.commit()
        cfg = parse_trading_settings(_make_raw_settings({'FAVOR_MIN_TIME_LEFT_SEC': '300', 'FAVOR_MAX_TIME_LEFT_SEC': '900', 'OUTS_MIN_TIME_LEFT_SEC': '300', 'OUTS_MAX_TIME_LEFT_SEC': '900'}))
        result = await load_eligible_markets(db_session, cfg, now)
        if result:
            ids = [m.market_id for m in result]
...
class TestStep4MarketGuards:
    ...
    async def test_step4_missing_token_ids_fails(self, db_session):
        ...
        cfg = parse_trading_settings(_make_raw_settings())
        result = await check_market_guards(db_session, market, cfg, TRADING_MODE_COMBINED, 600.0, datetime.now(timezone.utc))
    ...
    async def test_step4_duplicate_success_trade_fails(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        now = datetime.now(timezone.utc)
        db_session.add(TradeHistory(market_id='dup_market', asset='BTC', outcome_bought='YES', amount_usdc=10.0, remaining_shares=20.0, executed_price=0.6, predicted_flip_prob=0.7, active_features='mid_price', status='SUCCESS', mode='PAPER', created_at=now))
        await db_session.commit()
        market = _make_market(market_id='dup_market')
        cfg = parse_trading_settings(_make_raw_settings())
        result = await check_market_guards(db_session, market, cfg, TRADING_MODE_COMBINED, 600.0, now)
        assert not result.passed
    ...
    async def test_step4_existing_skipped_returned(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        now = datetime.now(timezone.utc)
        db_session.add(TradeHistory(market_id='skip_market', asset='BTC', outcome_bought='NONE', amount_usdc=0.0, remaining_shares=20.0, executed_price=0.0, predicted_flip_prob=0.0, active_features='', status='SKIPPED', error_msg='No signal', mode='PAPER', created_at=now))
        await db_session.commit()
...
class TestStep5aMLInference:
    ...
    def test_step5a_build_df_row_count(self):
        ...
        market = _make_market()
        df = build_inference_dataframe(market=market, history_snaps=snaps, fresh_yes_price=0.63, fresh_spread=0.02, global_max=0.75, start_time=datetime.now(timezone.utc), time_left_sec=600.0)
    ...
    def test_step5a_build_df_last_row_is_live(self):
        ...
        market = _make_market()
        df = build_inference_dataframe(market=market, history_snaps=snaps, fresh_yes_price=0.77, fresh_spread=0.015, global_max=0.8, start_time=datetime.now(timezone.utc), time_left_sec=300.0)
        last = df.iloc[-1]
    ...
    def test_step5a_build_df_no_recorded_at_column(self):
        ...
        from polyflip.trading.ml_inference import build_inference_dataframe
        market = _make_market()
        df = build_inference_dataframe(market=market, history_snaps=[], fresh_yes_price=0.6, fresh_spread=0.02, global_max=0.7, start_time=datetime.now(timezone.utc), time_left_sec=600.0)
    ...
    def test_step5a_price_distance_from_max_correct(self):
        ...
        from polyflip.trading.ml_inference import build_inference_dataframe
        market = _make_market()
        df = build_inference_dataframe(market=market, history_snaps=[], fresh_yes_price=0.6, fresh_spread=0.02, global_max=0.8, start_time=datetime.now(timezone.utc), time_left_sec=600.0)
        if 'price_distance_from_max' in df.columns:
            assert df.iloc[-1]['price_distance_from_max'] == pytest.approx(0.2, abs=0.001)
...
class TestStep6PreTradeValidator:
    ...
    def _make_decision(self, action='BUY_YES', buy_price=0.6, edge=0.1, bet=10.0):
        from polyflip.trading.decision_logic import TradeDecision
        return TradeDecision(action=action, buy_price=buy_price, bet_size_usdc=bet, strategy_type='ML_TREND', reason='test', edge=edge, p_win_effective=0.8, p_win_raw=0.8)
    ...
    async def test_step6_price_drift_too_large_fails(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        api_mock = AsyncMock()
        api_mock.get_market_prices = AsyncMock(return_value={'best_ask': 0.7})
        market = _make_market()
        cfg = parse_trading_settings(_make_raw_settings({'MAX_PRICE_DRIFT': '0.03'}))
        decision = self._make_decision(buy_price=0.6)
        result = await validate_pre_trade(db_session=db_session, api_client=api_mock, market=market, decision_obj=decision, cfg=cfg, asset_mode=TRADING_MODE_COMBINED, asset_min_edge=0.05, asset_max_price=0.95, p_flip=0.7, model_ver=1)
    ...
    async def test_step6_zero_bet_size_fails(self, db_session):
        ...
        market = _make_market()
        cfg = parse_trading_settings(_make_raw_settings({'BET_SIZING_MODE': 'fixed', 'TRADE_BET_SIZE_USDC': '0'}))
        decision = self._make_decision(buy_price=0.6, bet=0.0)
        result = await validate_pre_trade(db_session=db_session, api_client=api_mock, market=market, decision_obj=decision, cfg=cfg, asset_mode=TRADING_MODE_COMBINED, asset_min_edge=0.05, asset_max_price=0.95, p_flip=0.7, model_ver=1)
        assert not result.valid
    ...
    async def test_step6_valid_trade_returns_updated_buy_price(self, db_session):
        ...
        market = _make_market(current_yes_price=0.6)
        cfg = parse_trading_settings(_make_raw_settings())
        decision = self._make_decision(buy_price=0.6, edge=0.12, bet=10.0)
        result = await validate_pre_trade(db_session=db_session, api_client=api_mock, market=market, decision_obj=decision, cfg=cfg, asset_mode=TRADING_MODE_COMBINED, asset_min_edge=0.05, asset_max_price=0.95, p_flip=0.2, model_ver=1)
...
def _make_mock_exec(price):
    return TradeExecution(attempt_id=uuid.uuid4(), provider_order_id='test', provider_status='FILLED', status='FILLED', side='BUY', order_type='PAPER', token_id='test', original_requested_shares=Decimal('100'), submitted_shares=Decimal('100'), filled_shares=Decimal('100'), net_position_delta_shares=Decimal('100'), average_price=Decimal(str(price)), gross_quote_usdc=Decimal('10.0'), net_quote_usdc=Decimal('10.0'), liquidity_role='UNKNOWN', fees=ExecutionFees(platform_fee_usdc=None, builder_fee_usdc=None, network_fee_native=None, network_fee_symbol=None, network_fee_usdc=None, fee_source='UNKNOWN'), trade_ids=tuple(), transaction_hashes=tuple(), submitted_at=datetime.now(timezone.utc), observed_at=datetime.now(timezone.utc))

    def _make_validation(self, buy_price=0.6, bet=10.0, edge=0.12):
        from polyflip.trading.pre_trade_validator import PreTradeValidation
        return PreTradeValidation(valid=True, buy_price=buy_price, actual_bet_size=bet, edge=edge, skip_reason=None)

    def _make_decision_obj(self, action='BUY_YES', buy_price=0.6, edge=0.12):
        from polyflip.trading.decision_logic import TradeDecision
        return TradeDecision(action=action, buy_price=buy_price, bet_size_usdc=10.0, strategy_type='ML_TREND', reason='test', edge=edge, p_win_effective=0.8, p_win_raw=0.8)
    ...
    async def test_step7_creates_trade_history(self, db_session):
        ...
        cfg = parse_trading_settings(_make_raw_settings())
        await execute_and_record(db_session=db_session, trader=trader_mock, market=market, decision_obj=self._make_decision_obj(), validation=self._make_validation(), asset_mode=TRADING_MODE_COMBINED, active_features='mid_price', p_flip=0.7, model_ver=5, cfg=cfg, existing_skipped=None, start_time=datetime.now(timezone.utc))
        await db_session.commit()
        result = await db_session.execute(select(TradeHistory).where(TradeHistory.market_id == 'rec_m1'))
    ...
    async def test_step7_creates_slippage_log(self, db_session):
        ...
        cfg = parse_trading_settings(_make_raw_settings())
        await execute_and_record(db_session=db_session, trader=trader_mock, market=market, decision_obj=self._make_decision_obj(buy_price=buy_price), validation=self._make_validation(buy_price=buy_price), asset_mode=TRADING_MODE_COMBINED, active_features='mid_price', p_flip=0.7, model_ver=5, cfg=cfg, existing_skipped=None, start_time=datetime.now(timezone.utc))
        await db_session.commit()
        result = await db_session.execute(select(SlippageLog).where(SlippageLog.market_id == 'slip_m1'))
        slip = result.scalar_one_or_none()
        assert slip is not None
        assert slip.slippage == pytest.approx(exec_price - buy_price, abs=1e-05)
    ...
    async def test_step7_sets_stop_loss_when_enabled(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        trader_mock = AsyncMock()
        trader_mock.execute_trade = AsyncMock(return_value=_make_mock_exec(0.6))
        market = _make_market(market_id='sl_m1', asset='BTC')
        cfg = parse_trading_settings(_make_raw_settings({'STOP_LOSS_ENABLED': 'true', 'STOP_LOSS_PCT_FAVORITE': '40.0'}))
        await execute_and_record(db_session=db_session, trader=trader_mock, market=market, decision_obj=self._make_decision_obj(), validation=self._make_validation(), asset_mode=TRADING_MODE_COMBINED, active_features='mid_price', p_flip=0.7, model_ver=5, cfg=cfg, existing_skipped=None, start_time=datetime.now(timezone.utc))
        await db_session.commit()
        result = await db_session.execute(select(TradeHistory).where(TradeHistory.market_id == 'sl_m1'))
        trade = result.scalar_one_or_none()
        assert trade is not None
    ...
    async def test_step7_sets_take_profit_when_enabled(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        trader_mock = AsyncMock()
        trader_mock.execute_trade = AsyncMock(return_value=_make_mock_exec(0.5))
        market = _make_market(market_id='tp_m1', asset='BTC')
        cfg = parse_trading_settings(_make_raw_settings({'TAKE_PROFIT_ENABLED': 'true', 'TAKE_PROFIT_MULTIPLIER': '2.0'}))
        await execute_and_record(db_session=db_session, trader=trader_mock, market=market, decision_obj=self._make_decision_obj(), validation=self._make_validation(buy_price=0.5), asset_mode=TRADING_MODE_COMBINED, active_features='mid_price', p_flip=0.7, model_ver=5, cfg=cfg, existing_skipped=None, start_time=datetime.now(timezone.utc))
        await db_session.commit()
        result = await db_session.execute(select(TradeHistory).where(TradeHistory.market_id == 'tp_m1'))
        trade = result.scalar_one_or_none()
        assert trade is not None
        assert trade.take_profit_status == 'ACTIVE'
        assert trade.take_profit_price == pytest.approx(0.99, abs=0.01)
    ...
    async def test_step7_deletes_existing_skipped(self, db_session):
        ...
        from polyflip.constants import TRADING_MODE_COMBINED
        now = datetime.now(timezone.utc)
        existing = TradeHistory(market_id='del_m1', asset='BTC', outcome_bought='NONE', amount_usdc=0.0, remaining_shares=20.0, executed_price=0.0, predicted_flip_prob=0.0, active_features='', status='SKIPPED', error_msg='old skip', mode='PAPER', created_at=now)
        ...
        cfg = parse_trading_settings(_make_raw_settings())
        await execute_and_record(db_session=db_session, trader=trader_mock, market=market, decision_obj=self._make_decision_obj(), validation=self._make_validation(), asset_mode=TRADING_MODE_COMBINED, active_features='mid_price', p_flip=0.7, model_ver=5, cfg=cfg, existing_skipped=existing, start_time=now)
        await db_session.commit()
        result = await db_session.execute(select(TradeHistory).where(TradeHistory.market_id == 'del_m1', TradeHistory.status == 'SKIPPED'))
...
class TestStep8Orchestrator:
    ...
    async def test_step8_disabled_trading_returns_early(self, db_session):
        ...
        from polyflip.trading.engine import trade_worker_cycle
        now = datetime.now(timezone.utc)
        db_session.add(RuntimeSettings(key='TRADING_ENABLED', value='false', updated_at=now, updated_by='test'))
        await db_session.commit()
        trader_mock = AsyncMock()
        api_mock = AsyncMock()
        trader_mock.execute_trade = AsyncMock(side_effect=AssertionError('Не должен торговать'))
    ...
    async def test_step8_exception_does_not_propagate(self, db_session):
        ...
        trader_mock = AsyncMock()
        api_mock = AsyncMock()
        with patch('polyflip.trading.engine.load_eligible_markets', AsyncMock(side_effect=RuntimeError('DB down'))):
            try:
                await trade_worker_cycle(db_session, api_mock)
            except RuntimeError:
                pytest.fail('trade_worker_cycle должен поглощать исключения, а не пробрасывать')