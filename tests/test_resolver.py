...
async def test_resolver_skips_not_closed(db_session):
    ...
    res = await db_session.execute(
        select(MarketSnapshot).where(MarketSnapshot.market_id == "test_m1")
    )
...
async def test_resolver_marks_yes_and_calculates_flip(db_session):
    ...
    respx.get("https://gamma-api.polymarket.com/markets/test_m2").mock(
        return_value=httpx.Response(200, json={"closed": True, "answer": "NO"})
    )
    ...
    res = await db_session.execute(
        ...
        .where(MarketSnapshot.market_id == "test_m2")
...
async def test_resolve_trades_job_invalid(db_session):
    ...
    trade = TradeHistory(
        market_id="test_m_inv",
        asset="BTC",
        outcome_bought="YES",
        amount_usdc=10.0,
        remaining_shares=20.0,
        executed_price=0.5,
        predicted_flip_prob=0.1,
    ...
    res = await db_session.execute(
        select(TradeHistory).where(TradeHistory.market_id == "test_m_inv")
    )
