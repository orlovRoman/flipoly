...
async def test_engine_dynamic_no_flip_threshold(db_session):
    ...
    with patch("polyflip.trading.engine.PolymarketClient") as mock_api_cls:
        ...
        assert (
            trades[0].outcome_bought == "YES"
        ), "Должны были купить фаворита по тренду!"
...
async def test_recommended_thresholds_api(db_session):
    ...
    with patch(
        ...
        assert (
            response["per_asset"]["BTC"]["recommended_no_flip"] == 0.50
        )  # 0.65 - 0.15
        assert response["per_asset"]["BTC"]["is_auto_calibrated"] is True

        assert (
            "ETH" not in response["per_asset"]
        ), "Для ETH порога в БД нет, не должно быть в per_asset"
