"""build_final.py (T04) — final-period opportunities 08-31..09-13.

Identical logic to lgbm-favorite-flip-v1 build_opportunities.py (spec v1.1.0),
except the FINAL_LOCKED universe (market_end >= 2026-08-31) is INCLUDED with
fold='FINAL'. Pre-registered in market-policy-p0 spec v1.0.0 BEFORE opening.
Only the frozen P0 rule is applied here (no model training).
"""
import glob
import json
import os
...
OUTDIR = r"D:\market-policy-p0\out"
...
def main():
    ...
    stats = {"markets_total": len(uni), "markets_resolved": 0,
             ...
             "markets_no_end": 0, "markets_final": 0,
             "markets_fold_gap": 0, "markets_dev_skipped": 0,
             "markets_built": 0}
    ...
    for mk, u in uni.iterrows():
        ...
        if fold == "FINAL_LOCKED":
            fold = "FINAL"  # T04: pre-registered unlock of the final period
            stats["markets_final"] += 1
        elif fold == "FOLD_GAP":
            stats["markets_fold_gap"] += 1
            continue
        else:
            # dev folds must not leak into the final build
            stats["markets_dev_skipped"] += 1
            continue
        stats["markets_built"] += 1
        ct = float(u["ct_set"][0])
    ...
    op.to_parquet(os.path.join(OUTDIR, "FLIP_OPPORTUNITIES_FINAL.parquet"),
                  index=False)
    lad = op["reason_code"].value_counts().to_dict()
    lad["_markets"] = stats
    with open(os.path.join(OUTDIR, "COVERAGE_FINAL.json"), "w") as f:
