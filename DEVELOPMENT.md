...
Проект разбит на фазы. Фазы 0–5 — аналитика (MVP). Фаза 6 — торговля (полностью реализована). Фаза 7 — продвинутый бэктестинг (полностью реализована).
...
|---|---|---|
| 6. Торговый слой | 2-3 дня | ✅ Реализован |
| 7. Backtesting Framework | 2-3 дня | ✅ Реализован |

---

## Фаза 7: Backtesting Framework (2-3 дня)

**Цель:** Полноценный движок для симуляции торгов на исторических данных с учетом проскальзывания, ликвидности и комиссий.

### Задачи
- [x] Реализовать `runner.py` для перебора исторических снапшотов
- [x] Реализовать `market_replay.py` для эмуляции стакана
- [x] Добавить вычисление метрик PnL, Winrate, ROI
- [x] Добавить эндпоинты `/api/backtest/*` для управления бэктестами
- [x] Сделать UI страницу `/backtest` для визуализации результатов
...
Фаза 5 (Тесты)
    ...
    │
    ▼
Фаза 6 (Торговый слой — реализован)
    │
    ▼
Фаза 7 (Backtesting — реализован)
