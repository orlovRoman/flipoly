    ...
    try:
        ...
        for market in markets:
            ...
            try:
                ...
                val_max_price = raw_settings.get(
                    f"TRADE_MAX_PRICE_{market.asset.upper()}"
                )
                ...
                try:
                    if asset_mode == TRADING_MODE_ML:
                        ...
                        if not models_cache.models:
                            logger.warning(
                                "models_cache_empty_populating",
                                context="trade_worker_cycle",
                            )
                        ...
                        try:
                            ...
                            logger.error(
                                "decide_crypto_mode_import_error", error=str(e)
                            )
                            await save_or_update_skipped_trade(
                                db_session,
                                market,
                    ...
                    await save_or_update_skipped_trade(
                        db_session,
                        market,
                        f"Error calculating prediction: {e}",
                        0.0,
                        None,
                        start_time,
                        existing_skipped,
                ...
                if (
                    ...
                    skip_role = (
                        decision_res.decision_obj.decision_details.get("market_role")
                        if (
                            decision_res
                            and decision_res.decision_obj
                            and decision_res.decision_obj.decision_details
                        )
                        else None
                    )
                    from polyflip.trading.trade_recorder import (
                        _get_trade_active_features,
                    )
                    ...
                    await save_or_update_skipped_trade(
                        ...
                        lgbm_metadata=(
                            decision_res.lgbm_metadata if decision_res else None
                        ),
                        market_role=skip_role,
                        model_key=decision_res.used_model_key if decision_res else None,
                        confirm_model_key=(
                            decision_res.confirm_model_key if decision_res else None
                        ),
                        confirm_model_version=(
                            decision_res.confirm_model_version if decision_res else None
                        ),
                    )
                    continue
