...
from polyflip.constants import (
    TRADING_MODE_LIGHTGBM,
    TRADING_MODE_ML,
    TRADING_MODE_FAVORITE,
    TRADING_MODE_COMBINED,
)
...
def _get_trade_active_features(
    asset_mode: str, active_features_str: str, decision_obj: Any, asset_name: str = ""
) -> str:
    ...
    if asset_mode == TRADING_MODE_COMBINED:
        ...
        if not asset_name or asset_name.upper() in COMBINED_MODE_SUPPORTED_ASSETS:
            ...
            if (
                decision_obj
                and hasattr(decision_obj, "strategy_type")
    ...
    *,
    ...
    if existing_skipped:
        decision_changed = (
            ...
            or existing_skipped.predicted_flip_prob != p_flip_val
            or existing_skipped.edge != edge
            or existing_skipped.active_features != active_features
            or existing_skipped.lgbm_metadata != lgbm_metadata
            or (market_role and existing_skipped.market_role != market_role)
        )
        attribution_changed = (
        ...
        history = TradeHistory(
            ...
            mode=(
                "LIVE"
                if bool(
                    os.getenv("POLYGON_PRIVATE_KEY") and os.getenv("POLYGON_ADDRESS")
                )
                else "PAPER"
            ),
            edge=edge,
...
async def execute_and_record(
    ...
    *,
    ...
    p_flip_effective = (
        decision_obj.decision_details.get("p_flip_effective")
        if decision_obj.decision_details
        else None
    )
    ...
    history = TradeHistory(
        ...
        active_features=_get_trade_active_features(
            asset_mode, active_features, decision_obj, market.asset
        ),
    ...
    if cfg.stop_loss_enabled:
        ...
        stop_pct = (
            cfg.stop_loss_pct_outsider if is_outsider else cfg.stop_loss_pct_favorite
        )
    ...
    try:
        ...
        if result.disposition == EnqueueDisposition.DUPLICATE:
            raise EnqueueRejected(
                f"ActiveExecutionConflict: Market {market.market_id} already has an active OPEN request."
            )
        ...
        logger.warning(
            "enqueue_rejected_rolling_back", reason=str(e), market_id=market.market_id
        )
        raise
