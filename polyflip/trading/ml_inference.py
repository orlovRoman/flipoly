...
async def populate_models_cache(db_session: AsyncSession) -> None:
    ...
    load_stmt = select(ModelRegistry).where(
        ModelRegistry.is_active, ModelRegistry.asset.in_(to_load)
    )
    ...
    for m in models_to_load:
        try:
            ...
            m_feats = (
                [f.strip() for f in m.features.split(",") if f.strip()]
                if m.features
                else []
            )
        ...
        rows.append(
            {
                ...
                "market_duration_min": float(
                    getattr(snap, "market_duration_min", 60.0) or 60.0
                ),
            }
        )
    ...
    rows.append(
        {
            ...
            "market_duration_min": float(
                getattr(market, "market_duration_min", 60.0) or 60.0
            ),
        }
    )
...
def run_model_inference(
    ...
    if expected_features is not None:
        ...
        if expected_features != actual_features:
            ...
            raise ValueError(
                f"Feature order mismatch: expected {expected_features}, got {actual_features}"
            )
