...
async def validate_pre_trade(
    ...
    if decision_obj.action == "SKIP":
        return PreTradeValidation(
            ...
            actual_bet_size=0.0,
            edge=0.0,
            skip_reason=decision_obj.reason,
        )
    ...
    if (
        decision_obj.strategy_type == "LIGHTGBM_TREND"
        and actual_role == "OUTSIDER"
        and p_flip < cfg.flip_threshold
    ):
        return PreTradeValidation(
            valid=False,
            buy_price=buy_price,
    ...
    current_min_edge = (
        cfg.favorite_min_edge
        if (asset_mode == TRADING_MODE_FAVORITE and cfg.favorite_min_edge is not None)
        else asset_min_edge
    )
