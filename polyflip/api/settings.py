...
from polyflip.settings_registry import (
    registry_defaults,
    editable_keys as _registry_editable_keys,
)
...
router = APIRouter(
    prefix="/api/settings", tags=["Settings"], dependencies=[Depends(verify_api_key)]
)
...
async def get_all_settings(db: Optional[AsyncSession] = None):
    ...
    for asset in settings.asset_list:
        ...
        settings_dict[f"TRADING_MODE_{asset_upper}"] = db_settings.get(
            f"TRADING_MODE_{asset_upper}", ""
        )
        settings_dict[f"MIN_EDGE_{asset_upper}"] = db_settings.get(
            f"MIN_EDGE_{asset_upper}", ""
        )
        settings_dict[f"TRADE_MAX_PRICE_{asset_upper}"] = db_settings.get(
            f"TRADE_MAX_PRICE_{asset_upper}", ""
        )
        settings_dict[f"TRADE_FLIP_THRESHOLD_{asset_upper}"] = db_settings.get(
...
async def get_recommended_thresholds():
            ...
            select(RuntimeSettings).where(
                RuntimeSettings.key.in_(
                    [
                        ...
                        *[
                            f"TRADE_FLIP_THRESHOLD_{a.upper()}"
                            for a in settings.asset_list
                        ],
                        *[
                            f"AUTO_FLIP_THRESHOLD_{a.upper()}"
                            for a in settings.asset_list
                        ],
                    ]
                )
    ...
    dead_zone = float(
        db.get("DEAD_ZONE_WIDTH", getattr(settings, "DEAD_ZONE_WIDTH", 0.15))
    )
    ...
    return {
        "global": {
            "dead_zone": dead_zone,
            "current_no_flip": float(
                db.get("TRADE_NO_FLIP_THRESHOLD", settings.TRADE_NO_FLIP_THRESHOLD)
            ),
        },
        "per_asset": per_asset,
    }
    ...
    for key, val in payload.settings.items():
        ...
        try:
            ...
            await update_setting(
                key, SettingValue(value=val_str), request=request, db=db
            )
            saved.append(key)
...
async def update_security_setting(
    ...
    try:
        ...
        if existing:
            ...
            session.add(
                RuntimeSettings(
                    key=key, value=payload.value, updated_at=now, updated_by="api"
                )
            )
...
async def update_setting(
    ...
    if key == "STOP_LOSS_PCT":
        logger.warning(
            "deprecated_key_redirect",
            old_key="STOP_LOSS_PCT",
            new_key="STOP_LOSS_PCT_FAVORITE",
        )
    ...
    is_threshold_key = (
        ...
        or key.startswith("FLIP_THRESHOLD_")
        or key.startswith("TRADE_FLIP_THRESHOLD_")
        or key.startswith("NO_FLIP_THRESHOLD_")
        or key.startswith("TRADE_NO_FLIP_THRESHOLD_")
        or key.startswith("AUTO_FLIP_THRESHOLD_")
    )
    if is_threshold_key:
        if (
            key.startswith("FLIP_THRESHOLD_")
            or key.startswith("TRADE_FLIP_THRESHOLD_")
            or key.startswith("NO_FLIP_THRESHOLD_")
            or key.startswith("TRADE_NO_FLIP_THRESHOLD_")
            or key.startswith("AUTO_FLIP_THRESHOLD_")
            ...
            try:
                ...
                if val <= 0.0 or val >= 100.0:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Value for {key} must be between 0 and 100",
                    )
                ...
                if not (0.0 < val < 1.0):
                    raise HTTPException(
                        status_code=400,
                        detail=f"Value for {key} must be between 0.0 and 1.0",
                    )
                payload.value = str(round(val, 4))
            except ValueError:
                raise HTTPException(
                    status_code=400, detail=f"Value for {key} must be a number"
                )
        ...
        if key.startswith("MIN_EDGE_") and payload.value == "":
            ...
            try:
                ...
                if key == "FAVORITE_MIN_EDGE" and val < -100.0:
                    raise HTTPException(
                        status_code=400, detail=f"{key} must be ≥ -100%"
                    )
                if abs(val) > 1.0:
    ...
    if key == "FAVORITE_THRESHOLD":
        try:
            ...
            if not (0.01 <= val <= 0.99):
                raise HTTPException(
                    status_code=400,
                    detail="FAVORITE_THRESHOLD must be between 0.01 and 0.99",
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="FAVORITE_THRESHOLD must be a number"
            )
    ...
    if key == "DAILY_LOSS_LIMIT_USDC":
        try:
            ...
            if val >= 0.0:
                raise HTTPException(
                    status_code=400,
                    detail="DAILY_LOSS_LIMIT_USDC must be strictly negative (e.g., -100)",
                )
            if val < -100000.0:
                raise HTTPException(
                    status_code=400, detail="Daily loss limit is too large (> $100k)"
                )
        except ValueError:
            raise HTTPException(
                status_code=400,
                detail="Value for DAILY_LOSS_LIMIT_USDC must be a number",
            )
    ...
    if key == "TRADING_MODE" or key.startswith("TRADING_MODE_"):
        ...
        allowed = (
            allowed_per_asset if key.startswith("TRADING_MODE_") else allowed_global
        )
    ...
    if key == "TRADE_MAX_PRICE" or key.startswith("TRADE_MAX_PRICE_"):
        if key.startswith("TRADE_MAX_PRICE_") and payload.value == "":
            ...
            try:
                ...
                if not (0.01 <= val <= 0.99):
                    raise HTTPException(
                        status_code=400, detail=f"{key} must be between 0.01 and 0.99"
                    )
                payload.value = str(val)
            except ValueError:
                raise HTTPException(status_code=400, detail=f"{key} must be a number")
    ...
    if key == "FAVORITE_MODE_ENTRY_SEC":
        try:
            ...
            if not (30 <= val <= 600):
                raise HTTPException(
                    status_code=400,
                    detail="FAVORITE_MODE_ENTRY_SEC must be between 30 and 600 seconds",
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="FAVORITE_MODE_ENTRY_SEC must be an integer"
            )
            ...
            async with async_session() as session:
                if key == "FAVORITE_MAX_PRICE":
                    current_min_row = (
                        await session.execute(
                            select(RuntimeSettings).where(
                                RuntimeSettings.key == "FAVORITE_MIN_PRICE"
                            )
                        )
                    ).scalar_one_or_none()
                    current_min = (
                        float(current_min_row.value) if current_min_row else 0.55
                    )
                    if val <= current_min:
                        raise HTTPException(
                            status_code=400,
                            detail=f"FAVORITE_MAX_PRICE ({val}) must be > FAVORITE_MIN_PRICE ({current_min})",
                        )
                elif key == "FAVORITE_MIN_PRICE":
                    current_max_row = (
                        await session.execute(
                            select(RuntimeSettings).where(
                                RuntimeSettings.key == "FAVORITE_MAX_PRICE"
                            )
                        )
                    ).scalar_one_or_none()
                    current_max = (
                        float(current_max_row.value) if current_max_row else 0.95
                    )
                    if val >= current_max:
                        raise HTTPException(
                            status_code=400,
                            detail=f"FAVORITE_MIN_PRICE ({val}) must be < FAVORITE_MAX_PRICE ({current_max})",
                        )
    ...
    if key == "MAX_BET_SIZE_USDC":
        try:
            ...
            if val < 1.0:
                raise HTTPException(
                    status_code=400, detail="MAX_BET_SIZE_USDC must be >= 1.0"
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="MAX_BET_SIZE_USDC must be a number"
            )
    ...
    if key in ["TRADE_ON_FAVORITE", "TRADE_ON_FLIP"]:
        if payload.value.lower() not in ("true", "false"):
            raise HTTPException(
                status_code=400, detail=f"{key} must be 'true' or 'false'"
            )
    ...
    if key == "AUTO_DEAD_ZONE":
        if payload.value.lower() not in ("true", "false"):
            raise HTTPException(
                status_code=400, detail="AUTO_DEAD_ZONE must be 'true' or 'false'"
            )
        payload.value = payload.value.lower()
    ...
    if key == "LIVE_POLL_INTERVAL_SECONDS":
        try:
            ...
            if not (2 <= val <= 300):
                raise HTTPException(
                    status_code=400,
                    detail="LIVE_POLL_INTERVAL_SECONDS must be between 2 and 300 seconds",
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="LIVE_POLL_INTERVAL_SECONDS must be an integer"
            )

    if key == "STOP_LOSS_ENABLED":
        if payload.value.lower() not in ("true", "false"):
            raise HTTPException(
                status_code=400, detail="STOP_LOSS_ENABLED must be 'true' or 'false'"
            )
        payload.value = payload.value.lower()
    ...
    if key in ["STOP_LOSS_PCT", "STOP_LOSS_PCT_FAVORITE", "STOP_LOSS_PCT_OUTSIDER"]:
        try:
            ...
            if not (1.0 <= val <= 99.0):
                raise HTTPException(
                    status_code=400, detail="STOP_LOSS_PCT must be between 1.0 and 99.0"
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="STOP_LOSS_PCT must be a number"
            )
    ...
    if key == "STOP_LOSS_CHECK_SEC":
        try:
            ...
            if not (10 <= val <= 300):
                raise HTTPException(
                    status_code=400,
                    detail="STOP_LOSS_CHECK_SEC must be between 10 and 300 seconds",
                )
            payload.value = str(val)
        except ValueError:
            raise HTTPException(
                status_code=400, detail="STOP_LOSS_CHECK_SEC must be an integer"
            )
    ...
    async with SessionContext(db) as session:
        ...
        if setting:
            ...
            setting = RuntimeSettings(
                key=key, value=payload.value, updated_at=now, updated_by="dashboard"
            )
            session.add(setting)
