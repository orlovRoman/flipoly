...
BINANCE_SLEEP_SEC: float = (
    0.15  # 150ms между батчами → ~6 req/sec, хорошо в рамках лимита
)
    ...
    for symbol in symbols:
        for interval in intervals:
            ...
            try:
                ...
                logger.exception(
                    "historical_load_error", symbol=symbol, interval=interval
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Загрузка исторических свечей из Binance."
    )
    parser.add_argument(
        "--days", type=int, default=30, help="Количество дней истории (default: 30)"
    )
