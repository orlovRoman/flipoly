...
def build_crypto_features(
            ...
            [
                {
                    ...
                    "volume": c.volume,
                    "taker_buy_volume": (
                        c.taker_buy_volume if c.taker_buy_volume is not None else 0.0
                    ),
                }
                for c in candles
            ]
    ...
    if len(df) < min_candles:
        return CryptoFeatureVector(
            "", None, np.zeros((1, len(CRYPTO_FEATURE_COLUMNS))), False
        )
    ...
    c6_sum = (
        float(np.sum(cvd_raw[-6:])) if len(cvd_raw) >= 6 else float(np.sum(cvd_raw))
    )
