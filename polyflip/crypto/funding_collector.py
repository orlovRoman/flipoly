    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code in (429, 500, 502, 503, 504):
                logger.warning(
                    "funding_rate_fetch_temporary_error",
                    symbol=symbol,
        ...
        try:
            ...
            logger.warning(
                "funding_rate_invalid_data", symbol=symbol, error=str(e), data=data
            )
            return None
            ...
            if row:
                ...
                db.add(
                    RuntimeSettings(
                        ...
                        value=val,
                        updated_at=now,
                        updated_by="funding_collector",
                    )
                )
        await db.commit()

        logger.info(
            "funding_rate_stored", symbol=symbol, current=current_rate, ma3=ma3_rate
        )
        return current_rate
