...
async def main():
    ...
    config = BacktestConfig(assets=["BTC", "ETH", "SOL", "DOGE"], strategy_mode="ML")
    ...
    async with async_session() as db:
        ...
        for i, (market_id, replay) in enumerate(list(replays.items())[:5]):
            ...
            for t in ticks[:3]:
                ...
                print(
                    f"  Tick time_left={t.time_left_min:.2f}: p_flip={p_flip:.4f}, action={decision.action}, reason={decision.reason}"
                )
