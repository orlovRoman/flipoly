...
class SettingDef:
    ...
    default: str
    editable: bool = (
        True  # False — только для внутренних ключей (напр. TRADING_ENABLED)
    )
    description: str = ""
...
REGISTRY: list[SettingDef] = [
    ...
    SettingDef(
        "MAX_SINGLE_ORDER_USDC", "1.0", description="Максимальная сумма одной сделки"
    ),
    SettingDef(
    ...
    # --- Стоп-лосс позиции ---
    SettingDef(
        "STOP_LOSS_ENABLED", "false", description="Включить стоп-лосс открытых позиций"
    ),
    SettingDef(
    ...
    # --- Сайзинг ---
    SettingDef("TRADE_BET_SIZE_USDC", "5.0", description="Минимальная ставка (USDC)"),
    SettingDef("MAX_BET_SIZE_USDC", "50.0", description="Максимальная ставка (USDC)"),
    SettingDef(
        "BET_SIZING_MODE", "scaled", description="Режим расчёта размера: scaled | fixed"
    ),
    SettingDef(
        "LIQUIDITY_FRACTION",
        "0.05",
    ...
    # --- Edge ---
    SettingDef("MIN_EDGE", "0.05", description="Мин. edge для входа в сделку"),
    SettingDef(
        "MAX_BET_EDGE",
        "0.40",
    ...
    SettingDef(
        "FAVORITE_MIN_PRICE", "0.55", description="Мин. цена для входа в фаворита"
    ),
    SettingDef(
        "FAVORITE_MAX_PRICE", "0.95", description="Макс. цена для входа в фаворита"
    ),
    # --- Аутсайдер / NO ---
    SettingDef(
        "NO_MIN_EDGE", "0.04", description="Мин. edge для ставки на аутсайдера (NO)"
    ),
    SettingDef(
        "OUTSIDER_MAX_PRICE", "0.45", description="Макс. цена покупки аутсайдера"
    ),
    SettingDef(
        "TRADE_ON_FAVORITE",
        "true",
    ...
    SettingDef(
        "TRADE_ON_FLIP", "false", description="Торговать на флип (стратегия аутсайдера)"
    ),
    SettingDef(
        "FLIP_THRESHOLD", "0.60", description="Порог p_flip для входа в аутсайдера"
    ),
    SettingDef(
    ...
    SettingDef(
        "LR_MIN_FEATURES", "4", description="Мин. кол-во фич после отсева слабых"
    ),
    SettingDef(
        "LR_TRAIN_MAX_TIME_LEFT_MIN",
        "15.0",
    ...
    SettingDef(
        "LR_SAMPLE_WEIGHT_TAU",
        "5.0",
        description="Параметр tau для exp_decay взвешивания (в минутах). "
        "Сэмплы старше tau минут получают вес < 0.37.",
    ),
    # --- Цена входа ---
    SettingDef("TRADE_MIN_PRICE", "0.05", description="Мин. цена YES/NO для входа"),
    SettingDef("TRADE_MAX_PRICE", "0.95", description="Макс. цена YES/NO для входа"),
    # --- Активы ---
    SettingDef(
        "TRADE_ASSETS",
        "BTC,ETH",
    ...
    SettingDef(
        "ACTIVE_FEATURES",
        "mid_price,spread_pct,volume_5min,log_time_left,is_final_phase,price_deviation,high_price_final,price_velocity,price_momentum,spread_trend,volume_trend,price_distance_from_max,day_of_week",
        description="Список признаков для ML-модели",
    ),
    # --- Крипто ---
    SettingDef(
        "CRYPTO_MIN_EDGE", "0.05", description="Мин. edge для стратегии Crypto Trend"
    ),
    SettingDef(
        "USE_CRYPTO_CONFIRM",
        "false",
    ...
    # --- Бэктест ---
    SettingDef(
        "BACKTEST_MIN_EDGE", "0.04", description="Мин. edge для сигнала в бэктесте"
    ),
    SettingDef(
    ...
    SettingDef(
        "CRYPTO_LGBM_LEARNING_RATE", "0.05", description="Скорость обучения LightGBM"
    ),
    SettingDef(
    ...
    SettingDef(
        "CRYPTO_LGBM_MAX_DEPTH", "4", description="Максимальная глубина дерева LightGBM"
    ),
    SettingDef(
    ...
    SettingDef(
        "CRYPTO_LGBM_SUBSAMPLE", "0.8", description="Доля сэмплов для бутстрапа"
    ),
    SettingDef(
    ...
    SettingDef("CRYPTO_LGBM_REG_ALPHA", "0.1", description="L1 регуляризация LightGBM"),
    SettingDef(
        "CRYPTO_LGBM_REG_LAMBDA", "1.0", description="L2 регуляризация LightGBM"
    ),
    SettingDef(
