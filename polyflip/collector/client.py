            ...
            for event in events:
                ...
                for market in event.get("markets", []):
                    ...
                    if not isinstance(outcomes, list):
                        logger.debug(
                            "skipping_market_invalid_outcomes_type",
                            outcomes=outcomes,
                    ...
                    if outcomes_lower != ["up", "down"] and outcomes_lower != [
                        "yes",
                        "no",
                    ]:
                        logger.debug(
                            "skipping_market_unknown_outcomes",
                            outcomes=outcomes,
                            market_id=market.get("id"),
                        )
                        continue
        ...
        try:
            ...
            if response.status_code != 200:
                if response.status_code == 404:
                    logger.debug(
                        "clob_api_404_market_likely_closed", token_id=yes_token_id
                    )
                else:
                    logger.warning(
                        "clob_api_error",
            ...
            if best_ask <= best_bid:
                logger.warning(
                    "crossed_book", token_id=yes_token_id, bid=best_bid, ask=best_ask
                )
            ...
            logger.error(
                "error_fetching_clob_book", market_id=yes_token_id, error=str(e)
            )
            return {"error": f"API Error: {str(e)}"}

    async def get_recent_trades_volume(
        self, yes_token_id: str, minutes: int = VOLUME_WINDOW_MIN
    ) -> float:
        """
        ...
        try:
            ...
            response = await self.client.get(
                f"{self.CLOB_API}/trades", params={"token_id": yes_token_id}
            )
            if response.status_code != 200:
                if (
                    response.status_code != 401
                ):  # 401 means no CLOB API keys, don't spam
                    logger.warning(
                        "clob_trades_api_error",
                        token_id=yes_token_id,
                        status=response.status_code,
                    )
                return 0.0
            ...
            logger.error(
                "error_fetching_clob_trades", token_id=yes_token_id, error=str(e)
            )
            return 0.0
