...
def extract_final_outcome(market_data: dict) -> str | None:
    ...
    def normalize(val: Any) -> str | None:
        ...
        if val_str in ("YES", "UP"):
            return "YES"
        if val_str in ("NO", "DOWN"):
            return "NO"
        if val_str == "INVALID":
            return "INVALID"
        return None
                        ...
                        for snap in snapshots:
                            ...
                            if final_outcome in ("YES", "NO"):
                                if snap.mid_price == 0.5:
                                    ...
                                    snap.flip_vs_final = (
                                        market_believed_yes != actual_is_yes
                                    )
                            else:
                                snap.flip_vs_final = None
                    any_resolved = True
                    logger.info(
                        "market_prepared_to_resolve",
                        market_id=market_id,
                        outcome=final_outcome,
                    )
                except Exception as exc:
                    logger.exception(
                        "resolver_snapshot_failed", market_id=market_id, error=str(exc)
                    )

            except Exception as e:
                logger.error(
                    "error_preparing_market_resolve", market_id=market_id, error=str(e)
                )
