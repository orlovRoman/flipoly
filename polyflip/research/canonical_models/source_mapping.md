...
| strike_value | DB columns exist, 0 rows (verified). Gamma re-fetch probe 20/20 closed markets (5 assets, 07-01→09-09): NO strike-like numeric field on market objects; no event linkage; rules text ≠ strike. Report: `artifacts/.../strike_probe/report.md` | DATA_BLOCKED (Gamma path) | Chainlink-history path only after feed/history/rules confirmation (access not assumed). Binance substitution FORBIDDEN |
...
- Comparability correction: the first dev comparison mixed a 5-asset control (+14.94 of +16.13 gross on 09-09) against BTC-scoped CT. MAIN pair is now BTC-only control vs BTC control+CT on identical opportunity ids (self-checked); 5-asset control stays descriptive.
- PAPER reconciliation (working PAPER loop, not live orders): same-book L1 with 15 s freshness gate = 15/48 (30 STALE_BOOK, 2 PRICE_OUT_OF_RANGE on fresh books, 1 MISSING_BOOK). Side_match 1.0, regime_match 0.67; 3 usable BUYs; control +3.13 vs registered CT +0.85 (gross). Overlap-vs-reconstruction: 8 markets, final BUY/SKIP match, 0 BUY among them — SKIP-only agreement, weak evidence. Time/inputs/reasons/execution match NOT established.
- Live execution reality (trade_history for the 8 BUYs): 7/8 FAILED (6 POST_ONLY_REJECTED crossing + 2 edge-unavailable), 1 SUCCESS settled; trade_role NULL everywhere. Live runs post-only MAKER; frozen scenario comparison uses taker ladder-walk identically for both (observable, fair); live fills reconciled separately. Registered BUY is NOT a fill.
- Bundle gap (activation driver): decision_details carries spec/timing/features but NO input series/IDs, NO book prices, NO code sha. Exact CT reproduction from storage is impossible — hence regime_match < 1. See ACTIVATION.md.
- One BUY worked example (4375498): CT features reproduced bit-identically (ER 0.3077/sc 0.6154/ac −0.1653, 20 obs), side matches, ask diverges (0.09 live vs 0.2 recon, 29.5 s book age) — classified STALE_BOOK under the frozen gate, excluded from L1.
- Coverage (full local history): eligible = resolved markets with known end (35,552); decision-window hit 36.1% overall, 36.7% BTC (7,165), UP-leg 34.1%; uniform across hours; exact denominator recorded in `coverage_history.json`. The 15 s window at ~45 s cadence is a study selection, not a technicality.
- Old-history limits: new collection will NOT restore past books. Snapshots fit limited observed-quote analysis only; exact historical volume unavailable; strict symmetric analysis bounded by real DOWN-leg coverage (depth window 09-09..09-10). Strike absence blocks M-track, not CT/control.
