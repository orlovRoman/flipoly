...
| quotes both sides + depth | `market_snapshots` top-of-book both legs + `market_timestamp/received_timestamp/recorded_at`; `orderbook_depth_snapshots` (149k rows; 106k on 2026-09-09): full `bids/asks` jsonb ladders per token + `event_at/received_at/is_truncated/best_*_price+size/depth_usdc_*`, FK → snapshots.id, idx (market_id,token_id,received_at) | MAPPED | depth ladder = execution-grade fills via `orderbook_execution`; NO never via 1−YES |
